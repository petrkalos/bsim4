...
void malloc_memory(BSIM4model *model, CKTcircuit *ckt);
void memcpy_h_to_d(BSIM4model *model, CKTcircuit *ckt);
void memcpy_d_to_h(BSIM4model *model, CKTcircuit *ckt);
...
__global__ void d_BSIM4load(BSIM4instance *InstArray, CKTcircuit *ckt);
__device__ int BSIM4LoadOMP(BSIM4instance *here, CKTcircuit *ckt);
__device__ int BSIM4polyDepletion(double phi, double ngate, double epsgate,
		double coxe, double Vgs, double *Vgs_eff, double *dVgs_eff_dVg);
__device__ double d_CONSTvt0;

#define ccap qcap+1
#define MIN(a,b) ((a) < (b) ? (a) : (b))
#define MAX(a,b) ((a) > (b) ? (a) : (b))

__device__ double d_DEVfetlim(double vnew, double vold, double vto);
__device__ double d_DEVlimvds(double vnew, double vold);
__device__ double d_DEVpnjlim(double vnew, double vold, double vt, double vcrit, int *icheck);
__device__ int d_NIintegrate(CKTcircuit *ckt, double *geq, double *ceq, double cap, int qcap);

__device__ BSIM4instance *d_InstArray;
CKTcircuit *d_ckt;

/********* END DEVICE *******/

extern "C" void cuda_startKernel(GENmodel *inModel, CKTcircuit *ckt) {
	BSIM4model *model = (BSIM4model*) inModel;

	malloc_memory(model,ckt);
	memcpy_h_to_d(model,ckt);

	dim3 dimGrid(1,1);//model->BSIM4InstCount);
    dim3 dimBlock(1,1);

	d_BSIM4load<<<dimGrid,dimBlock>>>(d_InstArray,d_ckt);
	printError("Kernel Call");
...
void malloc_memory(BSIM4model *model, CKTcircuit *ckt){
	/* Malloc of Struct BSIM4instance*/
	cudaMalloc((void **)&d_InstArray,model->BSIM4InstCount*sizeof(BSIM4instance));

	/*for(int i=0;i<model->BSIM4InstCount;i++){
		cudaMalloc((void **)d_InstArray[0].BSIM4GEgePtr,sizeof(double));
	}*/

	/* Malloc of Struct CKTcircuit*/
	cudaMalloc((void **)&d_ckt,sizeof(CKTcircuit));
...
void memcpy_h_to_d(BSIM4model *model, CKTcircuit *ckt){
	/* Deep Copy of BSIM4instance*/
	for(int i=0;i<model->BSIM4InstCount;i++){
		cudaMemcpy(&d_InstArray[i],model->BSIM4InstanceArray[i],sizeof(BSIM4instance),cudaMemcpyHostToDevice);
	}


	/* Deep Copy of CKTcircuit*/
	cudaMemcpy(d_ckt,ckt,sizeof(CKTcircuit),cudaMemcpyHostToDevice);
...
__device__ int BSIM4LoadOMP(BSIM4instance *here, CKTcircuit *ckt) {
	...
	double Voxdepinv = 0.0, dVoxdepinv_dVg = 0.0, dVoxdepinv_dVd = 0.0,
			dVoxdepinv_dVb = 0.0;
	double VxNVt = 0.0, ExpVxNVt, Vaux = 0.0, dVaux_dVg = 0.0, dVaux_dVd = 0.0,
			dVaux_dVb = 0.0;
	...
	double vs, Fsevl, dvs_dVg, dvs_dVd, dvs_dVb, dFsevl_dVg, dFsevl_dVd,
			dFsevl_dVb;
...
***/
               ...
               {   vgs = d_DEVfetlim(vgs, *(ckt->CKTstate0 + here->BSIM4vgs), von);
                   vds = vgs - vgd;
                   vds = d_DEVlimvds(vds, *(ckt->CKTstate0 + here->BSIM4vds));
                   vgd = vgs - vds;
                   if (here->BSIM4rgateMod == 3)
                   {   vges = d_DEVfetlim(vges, *(ckt->CKTstate0 + here->BSIM4vges), von);
                       vgms = d_DEVfetlim(vgms, *(ckt->CKTstate0 + here->BSIM4vgms), von);
                       vged = vges - vds;
                       vgmd = vgms - vds;
                   }
                   ...
                   {   vdes = d_DEVlimvds(vdes, *(ckt->CKTstate0 + here->BSIM4vdes));
                       vses = -d_DEVlimvds(-vses, -(*(ckt->CKTstate0 + here->BSIM4vses)));
                   }

               }
               else
               {   vgd = d_DEVfetlim(vgd, vgdo, von);
                   vds = vgs - vgd;
                   vds = -d_DEVlimvds(-vds, -(*(ckt->CKTstate0 + here->BSIM4vds)));
                   vgs = vgd + vds;

                   if (here->BSIM4rgateMod == 3)
                   {   vged = d_DEVfetlim(vged, vgedo, von);
                       vges = vged + vds;
                       vgmd = d_DEVfetlim(vgmd, vgmdo, von);
                       vgms = vgmd + vds;
                   }
               ...
               if (vds >= 0.0)
               {   vbs = d_DEVpnjlim(vbs, *(ckt->CKTstate0 + here->BSIM4vbs),
                                   d_CONSTvt0, model->BSIM4vcrit, &Check);
                   vbd = vbs - vds;
                   if (here->BSIM4rbodyMod)
                   {   vdbs = d_DEVpnjlim(vdbs, *(ckt->CKTstate0 + here->BSIM4vdbs),
                                        d_CONSTvt0, model->BSIM4vcrit, &Check1);
                       vdbd = vdbs - vds;
                       vsbs = d_DEVpnjlim(vsbs, *(ckt->CKTstate0 + here->BSIM4vsbs),
                                        d_CONSTvt0, model->BSIM4vcrit, &Check2);
               ...
               {   vbd = d_DEVpnjlim(vbd, *(ckt->CKTstate0 + here->BSIM4vbd),
                                   d_CONSTvt0, model->BSIM4vcrit, &Check);
                   vbs = vbd + vds;
                   if (here->BSIM4rbodyMod)
                   {   vdbd = d_DEVpnjlim(vdbd, *(ckt->CKTstate0 + here->BSIM4vdbd),
                                        d_CONSTvt0, model->BSIM4vcrit, &Check1);
                       vdbs = vdbd + vds;
                       vsbdo = *(ckt->CKTstate0 + here->BSIM4vsbs)
                             - *(ckt->CKTstate0 + here->BSIM4vds);
                       vsbd = vsbs - vds;
                       vsbd = d_DEVpnjlim(vsbd, vsbdo, d_CONSTvt0, model->BSIM4vcrit, &Check2);
                       vsbs = vsbd + vds;
                       if ((Check1 == 0) && (Check2 == 0))
                           Check = 0;
                       else
	...
	if (SourceSatCurrent <= 0.0) {
		...
		case 0:
			...
			here->BSIM4gbs = SourceSatCurrent * (evbs + T1) / Nvtms
					+ ckt->CKTgmin;
			here->BSIM4cbs = SourceSatCurrent
					* (evbs + here->BSIM4XExpBVS - T1 - 1.0)
					+ ckt->CKTgmin * vbs_jct;
			break;
		case 1:
			T2 = vbs_jct / Nvtms;
			if (T2 < -EXP_THRESHOLD) {
				...
				here->BSIM4cbs = here->BSIM4IVjsmFwd - SourceSatCurrent
						+ T0 * (vbs_jct - here->BSIM4vjsmFwd)
						+ ckt->CKTgmin * vbs_jct;
			}
			break;
		case 2:
			if (vbs_jct < here->BSIM4vjsmRev) {
				...
				T1 = evbs - 1.0;
				T2 = here->BSIM4IVjsmRev
						+ here->BSIM4SslpRev * (vbs_jct - here->BSIM4vjsmRev);
				here->BSIM4gbs = devbs_dvb * T2 + T1 * here->BSIM4SslpRev
						+ ckt->CKTgmin;
				...
				here->BSIM4gbs = SourceSatCurrent
						* (devbs_dvb - model->BSIM4xjbvs * T3) + ckt->CKTgmin;
				here->BSIM4cbs = SourceSatCurrent
						* (evbs + here->BSIM4XExpBVS - 1.0
								- model->BSIM4xjbvs * T2)
						+ ckt->CKTgmin * vbs_jct;
			} else {
				here->BSIM4gbs = here->BSIM4SslpFwd + ckt->CKTgmin;
				here->BSIM4cbs = here->BSIM4IVjsmFwd
						+ here->BSIM4SslpFwd * (vbs_jct - here->BSIM4vjsmFwd)
						+ ckt->CKTgmin * vbs_jct;
			}
			break;
		default:
	...
	if (DrainSatCurrent <= 0.0) {
		...
		case 0:
			...
			/* WDLiu: Magic T1 in this form; different from BSIM4 beta. */
			here->BSIM4gbd = DrainSatCurrent * (evbd + T1) / Nvtmd
					+ ckt->CKTgmin;
			here->BSIM4cbd = DrainSatCurrent
					* (evbd + here->BSIM4XExpBVD - T1 - 1.0)
					+ ckt->CKTgmin * vbd_jct;
			break;
		case 1:
			T2 = vbd_jct / Nvtmd;
			if (T2 < -EXP_THRESHOLD) {
				...
				here->BSIM4cbd = here->BSIM4IVjdmFwd - DrainSatCurrent
						+ T0 * (vbd_jct - here->BSIM4vjdmFwd)
						+ ckt->CKTgmin * vbd_jct;
			}
			break;
		case 2:
			if (vbd_jct < here->BSIM4vjdmRev) {
				...
				T1 = evbd - 1.0;
				T2 = here->BSIM4IVjdmRev
						+ here->BSIM4DslpRev * (vbd_jct - here->BSIM4vjdmRev);
				here->BSIM4gbd = devbd_dvb * T2 + T1 * here->BSIM4DslpRev
						+ ckt->CKTgmin;
				...
				here->BSIM4gbd = DrainSatCurrent
						* (devbd_dvb - model->BSIM4xjbvd * T3) + ckt->CKTgmin;
				here->BSIM4cbd = DrainSatCurrent
						* (evbd + here->BSIM4XExpBVD - 1.0
								- model->BSIM4xjbvd * T2)
						+ ckt->CKTgmin * vbd_jct;
			} else {
				here->BSIM4gbd = here->BSIM4DslpFwd + ckt->CKTgmin;
				here->BSIM4cbd = here->BSIM4IVjdmFwd
						+ here->BSIM4DslpFwd * (vbd_jct - here->BSIM4vjdmFwd)
						+ ckt->CKTgmin * vbd_jct;
			}
			break;
		default:
	...
	Vth = model->BSIM4type * here->BSIM4vth0
			+ (pParam->BSIM4k1ox * sqrtPhis
					- pParam->BSIM4k1 * pParam->BSIM4sqrtPhi) * Lpe_Vb
			- here->BSIM4k2ox * Vbseff - Delt_vth - T2
			+ (pParam->BSIM4k3 + pParam->BSIM4k3b * Vbseff) * Vth_NarrowW + T1
			- DIBL_Sft;
	...
	if ((pParam->BSIM4dvtp4 == 0.0) || (pParam->BSIM4dvtp2factor == 0.0)) {
		...
		dDITS_Sft2_dVd = pParam->BSIM4dvtp2factor * pParam->BSIM4dvtp4 * 4.0
				* T10 / ((T0 + 1) * (T0 + 1));
		Vth -= DITS_Sft2;
	...
	BSIM4polyDepletion(T0, pParam->BSIM4ngate, T1, model->BSIM4coxe, vgs,
			&vgs_eff, &dvgs_eff_dvg);

	BSIM4polyDepletion(T0, pParam->BSIM4ngate, T1, model->BSIM4coxe, vgd,
			&vgd_eff, &dvgd_eff_dvg);
	...
	Weff = pParam->BSIM4weff
			- 2.0 * (pParam->BSIM4dwg * Vgsteff + pParam->BSIM4dwb * T9);
	...
	if (model->BSIM4mtrlMod && model->BSIM4mtrlCompatMod == 0)
		T14 = 2.0 * model->BSIM4type
				* (model->BSIM4phig - model->BSIM4easub - 0.5 * model->BSIM4Eg0
						+ 0.45);
	else
		T14 = 0.0;
	...
	/*high K mobility*/
		...
		dT11_dVg = -0.5 * pParam->BSIM4ucs * T11
				/ (0.5 + 0.5 * Vgsteff / VgsteffVth) / VgsteffVth;
	...
	if ((Rds == 0.0) && (Lambda == 1.0)) {
		...
		dVdsat_dVg = (dT1_dVg
				- (T1 * dT1_dVg - dT0_dVg * T2 - T0 * dT2_dVg) / T3
				- Vdsat * dT0_dVg) / T0;
		dVdsat_dVb = (dT1_dVb
				- (T1 * dT1_dVb - dT0_dVb * T2 - T0 * dT2_dVb) / T3
				- Vdsat * dT0_dVb) / T0;
	...
	Coxeff = epssub * model->BSIM4coxp / (epssub + model->BSIM4coxp * Tcen);
	...
	T3 = ueff / Leff;
	dbeta_dVg = CoxeffWovL * dueff_dVg
			+ T3 * (Weff * dCoxeff_dVg + Coxeff * dWeff_dVg);
	...
	dT0_dVg = -0.5
			* (Abulk * dVdseff_dVg - Abulk * Vdseff / Vgst2Vtm
	...
	dgche_dVg = (beta * dfgche1_dVg + fgche1 * dbeta_dVg - gche * dfgche2_dVg)
			/ fgche2;
	dgche_dVd = (beta * dfgche1_dVd + fgche1 * dbeta_dVd - gche * dfgche2_dVd)
			/ fgche2;
	dgche_dVb = (beta * dfgche1_dVb + fgche1 * dbeta_dVb - gche * dfgche2_dVb)
			/ fgche2;
	...
	if ((pParam->BSIM4pclm > MIN_EXP) && (diffVds > 1.0e-10)) {
		...
		Cclm = FP * PvagTerm * T0 * T1
				/ (pParam->BSIM4pclm * pParam->BSIM4litl);
		dCclm_dVg = Cclm
				* (dFP_dVg / FP + dPvagTerm_dVg / PvagTerm + dT0_dVg / T0
						+ dT1_dVg / T1);
		dCclm_dVb = Cclm
				* (dPvagTerm_dVb / PvagTerm + dT0_dVb / T0 + dT1_dVb / T1);
		dCclm_dVd = Cclm
				* (dPvagTerm_dVd / PvagTerm + dT0_dVd / T0 + dT1_dVd / T1);
		VACLM = Cclm * diffVds;
	...
	dIdsa_dVd = T0 * dIdl_dVd
			+ Idl * (1.0 - dVdseff_dVd - T9 * dVADIBL_dVd) / VADIBL;
	dIdsa_dVb = T0 * dIdl_dVb - Idl * (dVdseff_dVb + T9 * dVADIBL_dVb) / VADIBL;

	/* Add DITS to Ids */
	T9 = diffVds / VADITS;
	T0 = 1.0 + T9;
	dIdsa_dVg = T0 * dIdsa_dVg
			- Idsa * (dVdseff_dVg + T9 * dVADITS_dVg) / VADITS;
	dIdsa_dVd = T0 * dIdsa_dVd
			+ Idsa * (1.0 - dVdseff_dVd - T9 * dVADITS_dVd) / VADITS;
	...
	Gmb = (Ids * (dVdseff_dVb + dVdseff_dVg * dVgsteff_dVb) + Vdseff * tmp2)
			* dVbseff_dVb;
	...
	if (here->BSIM4IdovVds <= 1.0e-9)
		here->BSIM4IdovVds = 1.0e-9;

	/* Calculate Rg */
	if ((here->BSIM4rgateMod > 1) || (here->BSIM4trnqsMod != 0)
			|| (here->BSIM4acnqsMod != 0)) {
		...
		here->BSIM4gcrgb = pParam->BSIM4xrcrg1 * (dT0_dVb + tmp2) * dVbseff_dVb;
	...
	if (model->BSIM4gidlMod == 0) {
		...
		if ((pParam->BSIM4agidl <= 0.0) || (pParam->BSIM4bgidl <= 0.0)
			...
			if (T2 < 100.0) {
				Igidl = pParam->BSIM4agidl * pParam->BSIM4weffCJ * T1
						* exp(-T2);
				T3 = Igidl * (1.0 + T2) / T1;
				Ggidld = T3 * dT1_dVd;
				Ggidlg = T3 * dT1_dVg;
			} else {
				Igidl = pParam->BSIM4agidl * pParam->BSIM4weffCJ
						* 3.720075976e-44;
		...
		if (model->BSIM4mtrlMod == 0)
			...
			T1 = (-vds - vgd_eff - pParam->BSIM4egisl + pParam->BSIM4vfbsd)
					/ T0;
		...
		if ((pParam->BSIM4agisl <= 0.0) || (pParam->BSIM4bgisl <= 0.0)
			...
			if (T2 < 100.0) {
				Igisl = pParam->BSIM4agisl * pParam->BSIM4weffCJ * T1
						* exp(-T2);
				T3 = Igisl * (1.0 + T2) / T1;
				Ggisls = T3 * dT1_dVd;
				Ggislg = T3 * dT1_dVg;
			} else {
				Igisl = pParam->BSIM4agisl * pParam->BSIM4weffCJ
						* 3.720075976e-44;
		...
		/* GISL */
		if (model->BSIM4mtrlMod == 0)
			T1 = (-vds - pParam->BSIM4rgisl * vgd_eff - pParam->BSIM4egisl)
					/ T0;
		else
			T1 = (-vds - pParam->BSIM4rgisl * vgd_eff - pParam->BSIM4egisl
					+ pParam->BSIM4vfbsd) / T0;

		if ((pParam->BSIM4agisl <= 0.0) || (pParam->BSIM4bgisl <= 0.0)
				|| (T1 <= 0.0) || (pParam->BSIM4cgisl < 0.0))
			Igisl = Ggisls = Ggislg = Ggislb = 0.0;
		else {
			dT1_dVd = 1 / T0;
			...
			if (T2 < EXPL_THRESHOLD) {
				Igisl = pParam->BSIM4weffCJ * pParam->BSIM4agisl * T1
						* exp(-T2);
				T3 = Igisl / T1 * (T2 + 1);
				Ggisls = T3 * dT1_dVd;
				Ggislg = T3 * dT1_dVg;
		...
		if (model->BSIM4mtrlMod == 0)
			...
			T1 = (vds - pParam->BSIM4rgidl * vgs_eff - pParam->BSIM4egidl
					+ pParam->BSIM4vfbsd) / T0;

		if ((pParam->BSIM4agidl <= 0.0) || (pParam->BSIM4bgidl <= 0.0)
				|| (T1 <= 0.0) || (pParam->BSIM4cgidl < 0.0))
			Igidl = Ggidld = Ggidlg = Ggidlb = 0.0;
		else {
			dT1_dVd = 1 / T0;
			...
			T2 = pParam->BSIM4bgidl / T1;
			if (T2 < EXPL_THRESHOLD) {
				Igidl = pParam->BSIM4weffCJ * pParam->BSIM4agidl * T1
						* exp(-T2);
				T3 = Igidl / T1 * (T2 + 1);
				Ggidld = T3 * dT1_dVd;
				Ggidlg = T3 * dT1_dVg;
	...
	if (model->BSIM4igcMod) {
		...
		} else { /* T11 = pParam->BSIM4Bechvb * toxe; v4.7 */
		...
		dT7_dVd = -Vdseff * dPigcd_dVd - Pigcd * dVdseff_dVd
				+ dT7_dVg * dVgsteff_dVd;
		dT7_dVb = -Vdseff * dPigcd_dVb - Pigcd * dVdseff_dVb
				+ dT7_dVg * dVgsteff_dVb;
	...
	if (model->BSIM4igbMod) {
		...
		T11 = 4.97232e-7 * pParam->BSIM4weff * pParam->BSIM4leff
		...
		T5 = T12
				* (pParam->BSIM4aigbinv + T3 * Voxdepinv
						- T4 * Voxdepinv * Voxdepinv);
	...
	here->BSIM4ggisld = -(here->BSIM4ggisls + here->BSIM4ggislg
			+ here->BSIM4ggislb);
	here->BSIM4gIgbs =
			-(here->BSIM4gIgbg + here->BSIM4gIgbd + here->BSIM4gIgbb);
	...
	if (model->BSIM4tnoiMod == 0) {
		...
		here->BSIM4qinv = Coxeff * pParam->BSIM4weffCV * here->BSIM4nf
				* pParam->BSIM4leffCV * (Vgsteff - 0.5 * T0 + Abulk * T3);
	...
	if ((model->BSIM4xpart < 0) || (!ChargeComputationNeeded)) {
		...
		here->BSIM4cgbb = here->BSIM4csbb = here->BSIM4cdbb = here->BSIM4cbbb =
				0.0;
		here->BSIM4cqdb = here->BSIM4cqsb = here->BSIM4cqgb = here->BSIM4cqbb =
				0.0;
		here->BSIM4gtau = 0.0;
		goto finished;
		...
		CoxWL = model->BSIM4coxe * pParam->BSIM4weffCV * pParam->BSIM4leffCV
				* here->BSIM4nf;
		...
		if (Arg1 <= 0.0) {
			...
			if (model->BSIM4xpart > 0.5) { /* 0/100 Charge partition model */
				if (Vdsat <= Vds) { /* saturation region */
					T1 = Vdsat / 3.0;
					...
					here->BSIM4cggb = One_Third_CoxWL * (3.0 - dVdsat_dVg)
							* dVgs_eff_dVg;
					...
					here->BSIM4cgsb =
							-(here->BSIM4cggb + T11 + here->BSIM4cgdb);
					T6 = 1.0 / Vdsat;
					...
					here->BSIM4cdgb = (T7 * dAlphaz_dVg - T9 * dVdsat_dVg)
							* dVgs_eff_dVg;
					T12 = T7 * dAlphaz_dVb - T9 * dVdsat_dVb;
					here->BSIM4cddb = T4 * (3.0 - 6.0 * T2 - 3.0 * T5);
					here->BSIM4cdsb =
							-(here->BSIM4cdgb + T12 + here->BSIM4cddb);
					...
					here->BSIM4cbgb =
							-(here->BSIM4cggb + here->BSIM4cdgb + T10);
					here->BSIM4cbdb =
							-(here->BSIM4cgdb + here->BSIM4cddb + T12);
					here->BSIM4cbsb = -(here->BSIM4cgsb + here->BSIM4cdsb + T0);
				}
			} else if (model->BSIM4xpart < 0.5) { /* 40/60 Charge partition model */
				if (Vds >= Vdsat) { /* saturation region */
					...
					here->BSIM4cgdb = CoxWL * (T2 - 0.5 + 0.5 * T5);
					here->BSIM4cgsb =
							-(here->BSIM4cggb + here->BSIM4cgdb + tmp);
					...
					here->BSIM4cdgb = (T7 * dAlphaz_dVg - tmp1 * dVdsat_dVg)
							* dVgs_eff_dVg;
					...
					here->BSIM4cdsb =
							-(here->BSIM4cdgb + T10 + here->BSIM4cddb);
					...
					T10 = -4.0 * T4 * (T2 - 0.5 + 0.5 * T5) - here->BSIM4cddb;
					tmp = -(T10 + T11 + T12);

					here->BSIM4cbgb =
							-(here->BSIM4cggb + here->BSIM4cdgb + T12);
					here->BSIM4cbdb =
							-(here->BSIM4cgdb + here->BSIM4cddb + T10);
					here->BSIM4cbsb =
							-(here->BSIM4cgsb + here->BSIM4cdsb + tmp);
				}
			} else { /* 50/50 partitioning */
				if (Vds >= Vdsat) { /* saturation region */
					T1 = Vdsat / 3.0;
					qgate = CoxWL * (Vgs_eff - Vfb - pParam->BSIM4phi - T1);
					T2 = -Two_Third_CoxWL * Vgst;
					...
					here->BSIM4cdgb = (T0 * dVdsat_dVg - T7 * dAlphaz_dVg)
							* dVgs_eff_dVg;
					...
					here->BSIM4cbdb =
							-(here->BSIM4cgdb + 2.0 * here->BSIM4cddb);
					here->BSIM4cbsb =
							-(here->BSIM4cgsb + 2.0 * here->BSIM4cdsb);
		...
		if (model->BSIM4cvchargeMod == 0) {
			...
			if (T2 > EXP_THRESHOLD) {
				...
				dT10_dVb = T3 * dn_dVb
						- dT10_dVg * (dVth_dVb + Vgst * dn_dVb / n);
				dT10_dVd = T3 * dn_dVd
						- dT10_dVg * (dVth_dVd + Vgst * dn_dVd / n);
				dT10_dVg *= dVgs_eff_dVg;
			}
		...
		if (model->BSIM4capMod == 1) {
			...
			if (model->BSIM4xpart > 0.5) { /* 0/100 Charge petition model */
				T1 = T1 + T1;
				...
				Csb = CoxWL * (T5 * dVdseffCV_dVb + T6 * dAbulkCV_dVb)
						+ Csg * dVgsteff_dVb;
				Csg *= dVgsteff_dVg;
			} else if (model->BSIM4xpart < 0.5) { /* 40/60 Charge petition model */
				T1 = T1 / 12.0;
				T2 = 0.5 * CoxWL / (T1 * T1);
				T3 = Vgsteff
						* (2.0 * T0 * T0 / 3.0
				...
				T4 = -2.0 * qsrc / T1
						- T2
								* (Vgsteff * (3.0 * Vgsteff - 8.0 * T0 / 3.0)
										+ 2.0 * T0 * T0 / 3.0);
			...
			here->BSIM4cdsb = (Cgg + Cgd + Cgb + Cbg + Cbd + Cbb + Csg + Csd
					+ Csb);
		...
		/* Charge-Thickness capMod (CTM) begins */
			...
			if (pParam->BSIM4k1ox <= 0.0) {
				...
				Denomi = pParam->BSIM4moin * Vtm * pParam->BSIM4k1ox
						* pParam->BSIM4k1ox;
			...
			Cgd1 = CoxWLcen * T5 * dVdseffCV_dVd + Cgg1 * dVgsteff_dVd
					+ QovCox * dCoxeff_dVd;
			...
			} else if (model->BSIM4xpart < 0.5) { /* 40/60 partition */
				T2 = T2 / 12.0;
	...
	if ((here->BSIM4trnqsMod) || (here->BSIM4acnqsMod)) {
		...
		here->BSIM4cqbb =
				-(here->BSIM4cqgb + here->BSIM4cqdb + here->BSIM4cqsb);
		...
		*(ckt->CKTstate0+ here->BSIM4qcheq) = qcheq;
		...
		if (here->BSIM4trnqsMod) {
			error = d_NIintegrate(ckt, &geq, &ceq, 0.0, here->BSIM4qcheq);
			if (error)
				return (error);
		}
	}
	...
	*(ckt->CKTstate0+ here->BSIM4qdef) = qdef;
	...
	} else /* For both capMod == 1 and 2 */
	{
		...
		cgdo = pParam->BSIM4cgdo + T3
				- T3 * (1.0 - 1.0 / T4) * (0.5 - 0.5 * T0 / T1);
		qgdo = (pParam->BSIM4cgdo + T3) * vgdx
				- T3 * (T2 + 0.5 * pParam->BSIM4ckappad * (T4 - 1.0));
		...
		cgso = pParam->BSIM4cgso + T3
				- T3 * (1.0 - 1.0 / T4) * (0.5 - 0.5 * T0 / T1);
		qgso = (pParam->BSIM4cgso + T3) * vgsx
				- T3 * (T2 + 0.5 * pParam->BSIM4ckappas * (T4 - 1.0));
	}
	...
	if (here->BSIM4mode > 0) {
		if (here->BSIM4trnqsMod == 0) {
			...
			if (here->BSIM4rgateMod == 3) {
				...
				gcsgb = -(here->BSIM4cggb + here->BSIM4cbgb + here->BSIM4cdgb)
						* ag0;
				...
				gcggb = (here->BSIM4cggb + cgdo + cgso + pParam->BSIM4cgbo)
						* ag0;
			...
			gcsdb = -(here->BSIM4cgdb + here->BSIM4cbdb + here->BSIM4cddb)
					* ag0;
			gcssb = (here->BSIM4capbs + cgso
					- (here->BSIM4cgsb + here->BSIM4cbsb + here->BSIM4cdsb))
					* ag0;
			...
			if (!here->BSIM4rbodyMod) {
				...
				gcdbb = -(here->BSIM4cddb + here->BSIM4cdgb + here->BSIM4cdsb)
						* ag0;
				gcsbb = -(gcsgb + gcsdb + gcssb + gcsgmb)
						+ here->BSIM4capbs * ag0;
			...
			if (fabs(qcheq) <= 1.0e-5 * CoxWL) {
				...
				dxpart = qdrn / qcheq;
				Cdd = here->BSIM4cddb;
				Csd = -(here->BSIM4cgdb + here->BSIM4cddb + here->BSIM4cbdb);
				ddxpart_dVd = (Cdd - dxpart * (Cdd + Csd)) / qcheq;
				Cdg = here->BSIM4cdgb;
				Csg = -(here->BSIM4cggb + here->BSIM4cdgb + here->BSIM4cbgb);
				ddxpart_dVg = (Cdg - dxpart * (Cdg + Csg)) / qcheq;

				Cds = here->BSIM4cdsb;
				Css = -(here->BSIM4cgsb + here->BSIM4cdsb + here->BSIM4cbsb);
		...
		if (here->BSIM4trnqsMod == 0) {
			...
			if (here->BSIM4rgateMod == 3) {
				...
				gcdgb = -(here->BSIM4cggb + here->BSIM4cbgb + here->BSIM4cdgb)
						* ag0;
			...
			gcddb = (here->BSIM4capbd + cgdo
					- (here->BSIM4cgsb + here->BSIM4cbsb + here->BSIM4cdsb))
					* ag0;
			gcdsb = -(here->BSIM4cgdb + here->BSIM4cbdb + here->BSIM4cddb)
					* ag0;
			...
			if (!here->BSIM4rbodyMod) {
				...
				gcdbb = -(gcdgb + gcddb + gcdsb + gcdgmb)
						+ here->BSIM4capbd * ag0;
				gcsbb = -(here->BSIM4cddb + here->BSIM4cdgb + here->BSIM4cdsb)
						* ag0;
			...
			if (fabs(qcheq) <= 1.0e-5 * CoxWL) {
				...
				sxpart = qdrn / qcheq;
				Css = here->BSIM4cddb;
				Cds = -(here->BSIM4cgdb + here->BSIM4cddb + here->BSIM4cbdb);
				dsxpart_dVs = (Css - sxpart * (Css + Cds)) / qcheq;
				Csg = here->BSIM4cdgb;
				Cdg = -(here->BSIM4cggb + here->BSIM4cdgb + here->BSIM4cbgb);
				dsxpart_dVg = (Csg - sxpart * (Csg + Cdg)) / qcheq;

				Csd = here->BSIM4cdsb;
				Cdd = -(here->BSIM4cgsb + here->BSIM4cdsb + here->BSIM4cbsb);
	...
	if (here->BSIM4trnqsMod) {
		...
		*(ckt->CKTstate0 + here->BSIM4qcdump);
		error = d_NIintegrate(ckt, &geq, &ceq, 0.0, here->BSIM4qcdump);
		if (error)
		return(error);
	}
	...
	*(ckt->CKTstate0 + here->BSIM4qb) = qbulk;
	...
	error = d_NIintegrate(ckt, &geq, &ceq, 0.0, here->BSIM4qb);
	if (error)
		return (error);
	error = d_NIintegrate(ckt, &geq, &ceq, 0.0, here->BSIM4qg);
	if (error)
		return (error);
	error = d_NIintegrate(ckt, &geq, &ceq, 0.0, here->BSIM4qd);
	if (error)
		return (error);

	if (here->BSIM4rgateMod == 3) {
		error = d_NIintegrate(ckt, &geq, &ceq, 0.0, here->BSIM4qgmid);
		if (error)
			return (error);
	}

	if (here->BSIM4rbodyMod) {
		error = d_NIintegrate(ckt, &geq, &ceq, 0.0, here->BSIM4qbs);
		if (error)
			return (error);
		error = d_NIintegrate(ckt, &geq, &ceq, 0.0, here->BSIM4qbd);
		if (error)
			return (error);
	}
	...
	ceqqd = cqdrn - gcdgb * vgb - gcdgmb * vgmb + (gcddb + gcdbdb) * vbd
			- gcdbdb * vbd_jct + gcdsb * vbs;
	...
	if (here->BSIM4trnqsMod) {
		...
		ceqqd -= dxpart * T0
				+ T1
						* (ddxpart_dVg * vgb - ddxpart_dVd * vbd
								- ddxpart_dVs * vbs);
	...
	line900: if (here->BSIM4mode >= 0) {
		Gm = here->BSIM4gm;
		Gmbs = here->BSIM4gmbs;
		FwdSum = Gm + Gmbs;
		RevSum = 0.0;

		ceqdrn = model->BSIM4type
				* (cdrain - here->BSIM4gds * vds - Gm * vgs - Gmbs * vbs);
		ceqbd = model->BSIM4type
				* (here->BSIM4csub + here->BSIM4Igidl
						- (here->BSIM4gbds + here->BSIM4ggidld) * vds
						- (here->BSIM4gbgs + here->BSIM4ggidlg) * vgs
						- (here->BSIM4gbbs + here->BSIM4ggidlb) * vbs);
		ceqbs = model->BSIM4type
				* (here->BSIM4Igisl + here->BSIM4ggisls * vds
		...
		if (model->BSIM4igcMod) {
			...
			Istoteq =
					model->BSIM4type
							* (here->BSIM4Igs + here->BSIM4Igcs - gIstotg * vgs
									- here->BSIM4gIgcsd * vds
			...
			Idtoteq = model->BSIM4type
					* (here->BSIM4Igd + here->BSIM4Igcd - here->BSIM4gIgdg * vgd
							- here->BSIM4gIgcdg * vgs - here->BSIM4gIgcdd * vds
							- here->BSIM4gIgcdb * vbs);
		} else {
		...
		if (model->BSIM4igbMod) {
			...
			Ibtoteq = model->BSIM4type
					* (here->BSIM4Igb - here->BSIM4gIgbg * vgs
							- here->BSIM4gIgbd * vds - here->BSIM4gIgbb * vbs);
		} else
		...
		ceqdrn = -model->BSIM4type
				* (cdrain + here->BSIM4gds * vds + Gm * vgd + Gmbs * vbd);

		ceqbs = model->BSIM4type
				* (here->BSIM4csub + here->BSIM4Igisl
						+ (here->BSIM4gbds + here->BSIM4ggisls) * vds
						- (here->BSIM4gbgs + here->BSIM4ggislg) * vgd
						- (here->BSIM4gbbs + here->BSIM4ggislb) * vbd);
		ceqbd = model->BSIM4type
				* (here->BSIM4Igidl - here->BSIM4ggidld * vds
		...
		if (model->BSIM4igcMod) {
			...
			Istoteq = model->BSIM4type
					* (here->BSIM4Igs + here->BSIM4Igcd - here->BSIM4gIgsg * vgs
							- here->BSIM4gIgcdg * vgd + here->BSIM4gIgcdd * vds
							- here->BSIM4gIgcdb * vbd);
			...
			Idtoteq =
					model->BSIM4type
							* (here->BSIM4Igd + here->BSIM4Igcs
									- (here->BSIM4gIgdg + here->BSIM4gIgcsg)
											* vgd + here->BSIM4gIgcsd * vds
									- here->BSIM4gIgcsb * vbd);
		} else {
			gIstotg = gIstotd = gIstots = gIstotb = Istoteq = 0.0;
			gIdtotg = gIdtotd = gIdtots = gIdtotb = Idtoteq = 0.0;
		}
		...
		if (model->BSIM4igbMod) {
			...
			gIbtotb = here->BSIM4gIgbb;
			Ibtoteq = model->BSIM4type
					* (here->BSIM4Igb - here->BSIM4gIgbg * vgd
							+ here->BSIM4gIgbd * vds - here->BSIM4gIgbb * vbd);
		} else
			gIbtotg = gIbtotd = gIbtots = gIbtotb = Ibtoteq = 0.0;
	...
	if (model->BSIM4rdsMod == 1) {
		ceqgstot = model->BSIM4type
				* (here->BSIM4gstotd * vds + here->BSIM4gstotg * vgs
						+ here->BSIM4gstotb * vbs);
		...
		ceqgdtot = -model->BSIM4type
				* (here->BSIM4gdtotd * vds + here->BSIM4gdtotg * vgs
						+ here->BSIM4gdtotb * vbs);
	...
	if (!here->BSIM4rbodyMod) {
		here->BSIM4rhsbPrime = m
				* (ceqbd + ceqbs - ceqjd - ceqjs - ceqqb + Ibtoteq);
		here->BSIM4rhssPrime = m
				* (ceqdrn - ceqbs + ceqjs + ceqqg + ceqqb + ceqqd + ceqqgmid
						- ceqgstot + Istoteq);
	} else {
		...
		here->BSIM4rhssPrime = m
				* (ceqdrn - ceqbs + ceqjs + ceqqd + ceqqg + ceqqb + ceqqjd
						+ ceqqjs + ceqqgmid - ceqgstot + Istoteq);
	}
	...
	here->BSIM4_44 =
			m
					* (gdpr + here->BSIM4gds + here->BSIM4gbd + T1 * ddxpart_dVd
							- gdtotd + RevSum + gcddb + gbdpdp + dxpart * ggtd
							- gIdtotd);
	here->BSIM4_45 = m * (gdpr + gdtot);
	here->BSIM4_46 = m
			* (Gm + gcdgb - gdtotg + gbdpg - gIdtotg + dxpart * ggtg
					+ T1 * ddxpart_dVg);
	here->BSIM4_47 = m
			* (here->BSIM4gds + gdtots - dxpart * ggts + gIdtots
					- T1 * ddxpart_dVs + FwdSum - gcdsb - gbdpsp);
	here->BSIM4_48 = m
			* (gjbd + gdtotb - Gmbs - gcdbb - gbdpb + gIdtotb - T1 * ddxpart_dVb
					- dxpart * ggtb);
	...
	here->BSIM4_51 = m
			* (here->BSIM4gds + gstotd + RevSum - gcsdb - gbspdp
					- T1 * dsxpart_dVd - sxpart * ggtd + gIstotd);
	here->BSIM4_52 = m
			* (gcsgb - Gm - gstotg + gbspg + sxpart * ggtg + T1 * dsxpart_dVg
					- gIstotg);
	here->BSIM4_53 =
			m
					* (gspr + here->BSIM4gds + here->BSIM4gbs + T1 * dsxpart_dVs
							- gstots + FwdSum + gcssb + gbspsp + sxpart * ggts
							- gIstots);
	here->BSIM4_54 = m * (gspr + gstot);
	here->BSIM4_55 = m
			* (gjbs + gstotb + Gmbs - gcsbb - gbspb - sxpart * ggtb
	...
	if (here->BSIM4rbodyMod) {
		...
		here->BSIM4_81 =
				m
						* (here->BSIM4gbd - gcdbdb + here->BSIM4grbpd
								+ here->BSIM4grbdb);
		...
		here->BSIM4_87 = m
				* (here->BSIM4grbpd + here->BSIM4grbps + here->BSIM4grbpb);
		...
		here->BSIM4_91 =
				m
						* (here->BSIM4gbs - gcsbsb + here->BSIM4grbps
								+ here->BSIM4grbsb);
		...
		here->BSIM4_95 = m
				* (here->BSIM4grbsb + here->BSIM4grbdb + here->BSIM4grbpb);
	}
...
__device__ int d_NIintegrate(CKTcircuit *ckt, double *geq, double *ceq, double cap, int qcap)
{
    //char *ordmsg = "Illegal integration order";
    //char *methodmsg = "Unknown integration method";
